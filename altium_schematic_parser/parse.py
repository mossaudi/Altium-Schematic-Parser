...
def parse(input, format, **kwargs):
    ...
    lines = pattern.split(
        stream.read()[5:-1])  # lopping off first 4 bytes, and last byte, since they don't seem to matter?
    ...
    for i, current in enumerate(records_copy):
        ...
        if (s is None):
            ...
            try:
                ...
                if 0 <= ownerIndex < len(records_copy):
...
def determine_parts_list(schematic):
    # Changed 'is' to '==' for string comparison
    parts_list = {
...
def determine_net_list(schematic):
    ...
    for device in devices:
        ...
        # if a Wire, follow inconsistent location key names (X1 vs LOCATION.X, etc..)
            ...
            if coord_name_matches:
                # Add safety check for coordinate keys
                device['coords'] = []
                ...
                # If no valid coordinates found, set default
        ...
        # everything else, just convert the location values to ints
            ...
            # Add safety check for location keys
    ...
    for net in nets:
        ...
        if not net['name']:
            ...
            # Add safety checks for OWNERINDEX and parent existence
...
def is_connected(wire_a, wire_b):
    # Add safety check for coords existence
    if 'coords' not in wire_a or 'coords' not in wire_b:
        return False

    if wire_a["RECORD"] == "27":
        if len(wire_a['coords']) > 1:
    ...
    # Add safety check for TEXT key existence
...
def main(args):
    try:
        ...
        if args.output:
            with open(args.output, 'w') as json_file:  # Fixed: Use context manager
                json.dump(schematic, json_file, indent=4)
        else:
            print(json.dumps(schematic, indent=4))  # Fixed: Use json.dumps for consistent formatting
...
if __name__ == "__main__":
    ...
    parser.add_argument('-f', '--format', dest='format', default='all-hierarchy',
                        ...
                        all-hierarchy: All records in an owner/child "
